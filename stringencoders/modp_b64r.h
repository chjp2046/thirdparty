 ...
 * \file
 * <PRE>
 * High performance WEB-SAFE base64 encoder / decoder for rfc
 *
 ...
 *
 * Released under bsd license.  See modp_b64r.c for details.
 * </pre>
 *
 ...
 *
 * This uses "+" to "-", "/" to "_" as the replacement
 * alphabet.
 *
 ...
 *   at least modp_b64r_encode_len(len) bytes (see below)
 *   This will contain the null-terminated b64r encoded result
 ...
 * char* dest = (char*) malloc(modp_b64r_encode_len);
 * int len = modp_b64r_encode(dest, src, sourcelen);
 * if (len == -1) {
 *   printf("Error\n");
 * } else {
 *   printf("b64r = %s\n", dest);
 ...
 *
 */
int modp_b64r_encode(char* dest, const char* src, int len);
 ...
 * \endcode
 */
int modp_b64r_decode(char* dest, const char* src, int len);
...
namespace modp {
    ...
    inline std::string b64r_encode(const char* s, size_t len)
    {
        ...
        int d = modp_b64r_encode(const_cast<char*>(x.data()), s,
                                 static_cast<int>(len));
    ...
    inline std::string b64r_decode(const char* src, size_t len)
    {
        ...
        int d = modp_b64r_decode(const_cast<char*>(x.data()), src,
                                 static_cast<int>(len));
        if (d < 0) {
            x.clear();
        } else {
