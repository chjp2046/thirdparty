...
#include "thirdparty/zookeeper-3.3.3/src/c/include/zookeeper_version.h"
...
DECLARE_bool(zk_test_env_enable);
DECLARE_string(zk_test_proxy_host);
...
enum ZOO_ERRORS {
  ...
  ZNOQUOTA = -119, /*!<There is no quota left for current user. */
  ZSERVEROVERLOAD = -120,  /* server overload */
};
...
/**
 * \brief judge if is a valid zk path.
 * \param path zk path
 * \param flags node type flags.
 * \param create if this function is called when creating a node.
 * \return 1 if is valid, else 0.
 */
int isValidPath(const char* path, const int flags, bool create = false);
 ...
 * \param generator taas credential generator, used for taas to generate credential
 ...
 *   internally and can be null.
 * \param flags reserved for future use. Should be set to zero.
 * \param credential taas credential , used to authenticate by server.
 * \param credential_len the length of credential.
 * \return a pointer to the opaque zhandle structure. If it fails to create
 * a new zhandle the function returns NULL and the errno variable
 * indicates the reason.
 */
...
ZOOAPI zhandle_t *zookeeper_init(const char *host, watcher_fn fn,
  int recv_timeout, const clientid_t *clientid, void *context, int flags,
  const char* credential = NULL, int credential_len = 0);
...
ZOOAPI void zoo_set_debug_level(ZooLogLevel logLevel)
    DEPRECATED_MESSAGE("This function is unused now, please remove it from your code.");
