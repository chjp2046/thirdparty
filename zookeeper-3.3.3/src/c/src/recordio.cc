...
{
    struct buff_struct *priv = static_cast<buff_struct*>(oa->priv);
    int32_t i = htonl(*d);
...
{
    struct buff_struct *priv = static_cast<buff_struct*>(ia->priv);
        ...
        struct buffer *b)
{
    ...
    b->buff = static_cast<char*>(malloc(b->len));
    if (!b->buff) {
        return -ENOMEM;
    }
...
{
    struct iarchive *ia = static_cast<iarchive*>(malloc(sizeof(*ia)));
    struct buff_struct *buff = static_cast<buff_struct*>(malloc(sizeof(struct buff_struct)));
    if (!ia) return 0;
    if (!buff) {
        free(ia);
        return 0;
...
{
    struct oarchive *oa = static_cast<oarchive*>(malloc(sizeof(*oa)));
    struct buff_struct *buff = static_cast<buff_struct*>(malloc(sizeof(struct buff_struct)));
    if (!oa) return 0;
    if (!buff) {
        free(oa);
    ...
    *oa = oa_default;
    buff->off = 0;
    buff->buffer = static_cast<char*>(malloc(128));
    buff->len = 128;
    oa->priv = buff;
    return oa;
}
...
{
    if (free_buffer) {
        struct buff_struct *buff = static_cast<buff_struct *>((*oa)->priv);
        if (buff->buffer) {
            free(buff->buffer);
        }
    }
