...
DEFINE_bool(zk_test_env_enable, false, "if use test environment.");
DEFINE_string(zk_test_proxy_host, "", "test environment zk proxy host address.");
...
static struct ZKACL _OPEN_ACL_UNSAFE_ACL[] = {{0x1f, {"world", "anyone"}}};
static struct ZKACL _READ_ACL_UNSAFE_ACL[] = {{0x01, {"world", "anyone"}}};
static struct ZKACL _CREATOR_ALL_ACL_ACL[] = {{0x1f, {"auth", ""}}};
...
static ssize_t send_nocancel(int fd, const void* buffer, size_t length, int flags)
{
    ssize_t result;
    do {
...
static ssize_t recv_nocancel(int fd, void* buffer, size_t length, int flags)
{
    ssize_t result;
    do {
...
static void clear_auth_completion(auth_completion_list_t *a_list) {
    a_list->completion = NULL;
    a_list->auth_data = NULL;
    return;
}

static void set_auth_completion(auth_completion_list_t* a_list, void_completion_t* completion,
                                const char *data) {
...
static void get_auth_completions(auth_list_head_t *auth_list, auth_completion_list_t *a_list) {
    ...
    if (element->completion) {
        set_auth_completion(a_list, &element->completion, element->data);
        // when you add completion, set it to null.
...
static void set_auth(auth_list_head_t *auth_list, auth_info *add_el) {
    free_auth_info(auth_list);
...
// Caculate interval between two timestamps, in milliseconds.
...
int getaddrs(zhandle_t *zh)
{
    ...
    while(host) {
        ...
        *port_spec = '\0';
        ...
        if (!he) {
            LOG_ERROR(("failed to call gethostbyname: %s, for the reason: %s\n",
        ...
        if ((rc = getaddrinfo(host, port_spec, &hints, &res0)) != 0) {
            ...
            if (rc != 0) {
                ...
                LOG_ERROR(("Failed to resolve address: %s, for the reason: %s\n.",
        ...
        for (res = res0; res; res = res->ai_next) {
            ...
            if (zh->addrs_count == alen) {
                ...
                zh->addrs = static_cast<sockaddr_storage*>(tmpaddr);
            }
...
static void log_env() {
  ...
  LOG_DEBUG(("Client environment:zookeeper.version=%s", PACKAGE_STRING));

#ifdef HAVE_GETHOSTNAME
  gethostname(buf, sizeof(buf));
  LOG_DEBUG(("Client environment:host.name=%s", buf));
#else
  LOG_DEBUG(("Client environment:host.name=<not implemented>"));
#endif

#ifdef HAVE_SYS_UTSNAME_H
  uname(&utsname);
  LOG_DEBUG(("Client environment:os.name=%s", utsname.sysname));
  LOG_DEBUG(("Client environment:os.arch=%s", utsname.release));
  LOG_DEBUG(("Client environment:os.version=%s", utsname.version));
#else
  LOG_DEBUG(("Client environment:os.name=<not implemented>"));
  LOG_DEBUG(("Client environment:os.arch=<not implemented>"));
  LOG_DEBUG(("Client environment:os.version=<not implemented>"));
#endif

#ifdef HAVE_GETLOGIN
  LOG_DEBUG(("Client environment:user.name=%s", getlogin()));
#else
  LOG_DEBUG(("Client environment:user.name=<not implemented>"));
#endif

#if defined(HAVE_GETUID) && defined(HAVE_GETPWUID_R)
  uid = getuid();

  // getpwuid_r return 0 when no error occurs, not says pw has been found
  // pwp points to the found entry or NULL when no entry is found
  if (!getpwuid_r(uid, &pw, buf, sizeof(buf), &pwp) && pwp) {
    LOG_DEBUG(("Client environment:user.home=%s", pw.pw_dir));
  } else {
    LOG_DEBUG(("Client environment:user.home=<NA>"));
  }
#else
  LOG_DEBUG(("Client environment:user.home=<not implemented>"));
#endif

#ifdef HAVE_GETCWD
  if (!getcwd(buf, sizeof(buf))) {
    LOG_DEBUG(("Client environment:user.dir=<toolong>"));
  } else {
    LOG_DEBUG(("Client environment:user.dir=%s", buf));
  }
#else
  LOG_DEBUG(("Client environment:user.dir=<not implemented>"));
#endif
}
...
static zhandle_t *zookeeper_init_internal(const char *host, watcher_fn watcher,
  ...
  int flags, const char *credential, int credential_len,
  common::CredentialGenerator* generator)
{
    int errnosave = 0;
    zhandle_t *zh = NULL;
    ...
    LOG_INFO(("Initiating client connection, host=%s, sessionTimeout=%d", host, recv_timeout));

    zh = static_cast<zhandle_t*>(calloc(1, sizeof(*zh)));
    if (!zh) {
        return 0;
    }
    zh->fd = -1;
    ...
    if (recv_timeout > 0) {
        ...
        zh->recv_timeout = 4000;
    }
    if (generator != NULL) {
        zh->generator = generator;
    } else {
        // default instance may be null if no flag or flag error.
        zh->generator = common::CredentialGenerator::DefaultInstance();
    }
    struct buffer auth;
    auth_info *authinfo;
    if (credential != NULL && credential_len > 0) {
        auth.buff = static_cast<char*>(calloc(1, credential_len));
        if (auth.buff == 0) {
            goto abort;
        }
        memcpy(auth.buff, credential, credential_len);
        auth.len = credential_len;
    } else {
        auth.buff = 0;
        auth.len = 0;
    }
    ...
    authinfo->scheme = strdup("taas");
    authinfo->auth = auth;
    authinfo->completion = 0;
    authinfo->data = 0;
    set_auth(&zh->auth_h, authinfo);
    zoo_unlock_auth(zh);
    ...
    gettimeofday(&end, 0);
    interval = calculate_interval(&begin, &end);
    // more than 2 seconds
    if (interval > 2 * 1000) {
        LOG(WARNING) << "dns lookup takes a long time: " << interval;
    }
    zh->connect_index = 0;
    if (clientid) {
    ...
    zh->primer_buffer.buffer = NULL;
    zh->primer_buffer.curr_offset = 0;
    zh->primer_buffer.len = 0;
...
zhandle_t *zookeeper_init(const char *host, watcher_fn watcher,
  int recv_timeout, const clientid_t *clientid, void *context,
  int flags, common::CredentialGenerator* generator)
{
    return zookeeper_init_internal(host, watcher, recv_timeout, clientid,
                          context, flags, NULL, 0, generator);
}
...
zhandle_t *zookeeper_init(const char *host, watcher_fn watcher,
  ...
  int flags, const char *credential, int credential_len)
{
    return zookeeper_init_internal(host, watcher, recv_timeout, clientid, context,
                          flags, credential, credential_len, NULL);
}
...
{
    buffer_list_t *buffer = static_cast<buffer_list_t*>(calloc(1, sizeof(*buffer)));
    if (buffer == 0)
        return 0;
...
static int send_buffer(int fd, buffer_list_t *buff)
{
    ...
    if (off < 4) {
        ...
        int nlen = htonl(len);
        char *b = (char*)&nlen;
        rc = send_nocancel(fd, b + off, sizeof(nlen) - off, 0);
        if (rc == -1) {
            if (errno != EAGAIN) {
                return -1;
            } else {
    ...
    if (off >= 4) {
        ...
        off -= sizeof(buff->len);
        rc = send_nocancel(fd, buff->buffer + off, len - off, 0);
        if (rc == -1) {
            if (errno != EAGAIN) {
                return -1;
            }
...
static int recv_buffer(int fd, buffer_list_t *buff)
{
    ...
    if (off < 4) {
        ...
        rc = recv_nocancel(fd, buffer+off, sizeof(int)-off, 0);
        //fprintf(LOGSTREAM, "rc = %d, off = %d, line %d\n", rc, off, __LINE__);
        switch(rc) {
        case 0:
            errno = EHOSTDOWN;
    ...
    if (buff->buffer) {
        ...
        rc = recv_nocancel(fd, buff->buffer+off, buff->len-off, 0);
        switch(rc) {
        case 0:
            errno = EHOSTDOWN;
        case -1:
    ...
    while (tmp_list.head) {
        ...
        if (cptr->c.data_result == SYNCHRONOUS_MARKER) {
            ...
            if(cptr->xid == PING_XID){
                ...
                bptr = static_cast<buffer_list_t*>(calloc(sizeof(*bptr), 1));
                assert(bptr);
...
char* get_local_ip(void)
{
    ...
    size_t i = 0;
    struct ifreq* req = (struct ifreq*)buffer;
        ...
        const char* format, ...)
{
    ...
    // Print session info only when session expired.
    // In other case, we only print info in debug mode to avoid logs block current thread.
    if (rc == ZSESSIONEXPIRED) {
        LOG_ERROR(("(%#llx) Server [%s], Client[%s:%d], zk retcode=%d,"
                "errno=%d(%s): %s, Current idle recv = %d, idle send = %d",
                zh->client_id.client_id,
                format_current_endpoint_info(zh),
                get_local_ip(), zh->port,
                rc, errno, strerror(errno), buf, idle_recv, idle_send));
    } else if (rc == ZAUTHFAILED || rc == ZNOQUOTA || rc == ZSERVEROVERLOAD) {
        LOG_WARN(("Server [%s], Client[%s:%d], connect error, retcode=%d.",
                  format_current_endpoint_info(zh), get_local_ip(), zh->port, rc));
    } else {
        LOG_DEBUG(("(%#llx) Server [%s], Client[%s:%d], zk retcode=%d,"
                "errno=%d(%s): %s, Current idle recv = %d, idle send = %d",
                zh->client_id.client_id,
                format_current_endpoint_info(zh),
                get_local_ip(), zh->port,
...
static void auth_completion_func(int rc, zhandle_t* zh)
{
    ...
    zoo_unlock_auth(zh);
    if (rc) {
        LOG_DEBUG(("(%#llx) Authentication scheme %s failed. Connection closed.",
                    zh->client_id.client_id, zh->auth_h.auth->scheme));
    }
    else {
        LOG_DEBUG(("Authentication scheme %s succeeded", zh->auth_h.auth->scheme));
    }
    a_tmp = &a_list;
    // chain call user's completion function
    if (a_tmp->completion != NULL) {
...
static int send_info_packet(zhandle_t *zh, auth_info* auth) {
    struct oarchive *oa;
    struct RequestHeader h = { AUTH_XID, SETAUTH_OP };
...
static int send_auth_info(zhandle_t *zh) {
    ...
    zoo_lock_auth(zh);
    auth = zh->auth_h.auth;
    // in order to be compatible with old server.
    if (auth == NULL || strncmp(auth->scheme, "taas", strlen("taas") + 1) == 0) {
        zoo_unlock_auth(zh);
        return ZOK;
    }
...
{
    struct oarchive *oa;
    struct RequestHeader h = { SET_WATCHES_XID, SETWATCHES_OP };
...
static int serialize_prime_connect(struct connect_req *req, char* buffer){
    ...
    int offset = 0;
    int credential_len = req->credential_len;
    ...
    req->credential_len = htonl(req->credential_len);
    memcpy(buffer + offset, &req->credential_len, sizeof(req->credential_len));
    offset = offset + sizeof(req->credential_len);

    memcpy(buffer + offset, req->credential, credential_len);
    return 0;
}

 static int deserialize_prime_response(struct prime_struct *req,
                                       struct _buffer_list *primer_buffer){
     //this should be the order of deserialization
     req->len = primer_buffer->len;
     char* buffer = primer_buffer->buffer;
     int offset = 0;
     ...
     req->error_code = ZOK;
     // if offset < req->len, we will get error_code from prime_response.
     if (offset < req->len) {
        memcpy(&req->error_code, buffer + offset, sizeof(req->error_code));
        req->error_code = ntohl(req->error_code);
     }
     // the buffer is dynamic allocate in function recv_buffer according to
     // the length of ConnecteResponse. free it here.
     free(buffer);
     // offset must set to zero, or reconncetion will fail.
     primer_buffer->buffer = NULL;
     primer_buffer->curr_offset = 0;
     return 0;
 }

static int prime_connection(zhandle_t *zh)
{
    LOG_DEBUG(("(%#llx) have a handshake with server [%s], client id [%s:%d] ",
                zh->client_id.client_id,
                format_endpoint_info(&zh->addrs[zh->connect_index]),
                get_local_ip(), zh->port));
    int rc;
    auth_info *auth = zh->auth_h.auth;
    ...
    if (zh->generator == NULL) {
        // no flags or flags error, use credential given by param.
        req.credential_len = auth->auth.len;
        req.credential = auth->auth.buff;
        if (req.credential_len == 0) {
            VLOG(5) << "No generator and credential is empty. Login as guest.";
        }
    } else {
        if (!zh->generator->Generate(&credential)) {
            credential = "";
            // now taas generator failure treat it as guest.
            LOG(WARNING) << "Failed to generate the credential. Login as guest.";
        } else if (credential.empty()) {
            VLOG(5) << "Taas succeeded to generate credential, but it's empty.";
        }
        req.credential_len = credential.size();
        req.credential = credential.data();
    }

    // alloc len buffer for serialize connect request. it includes a fix parts and a
    // variable parts base on credential's length.
    int len = sizeof(struct connect_req) - sizeof(char*) + req.credential_len;

    /*this is the buffer to serialize req into*/
    char* buffer_req = static_cast<char*>(calloc(1, len));
    if (buffer_req == NULL) {
        LOG_ERROR(("out of memory."));
        return handle_socket_error_msg(zh, __LINE__, ZSYSTEMERROR,
                "failed to allocate buffer for credential:%s", strerror(errno));
    }

    int hlen = htonl(len);
    /* We are running fast and loose here, but this string should fit in the initial buffer! */
    rc = send_nocancel(zh->fd, &hlen, sizeof(len), 0);
    serialize_prime_connect(&req, buffer_req);
    rc=rc<0 ? rc : send_nocancel(zh->fd, buffer_req, len, 0);
    // already sent, free buffer_req
    free(buffer_req);
    if (rc<0) {
...
static struct timeval get_timeval(int interval)
{
 ...
 int send_ping(zhandle_t* zh)
 {
    ...
    struct RequestHeader h = { PING_XID, PING_OP };
    ...
    *interest = 0;
    ...
    if (*fd == -1) {
        if (zh->connect_index == zh->addrs_count) {
            ...
            int val = fcntl(zh->fd, F_GETFD);
            val |= FD_CLOEXEC;
            fcntl(zh->fd, F_SETFD, val);

            int len;
            struct sockaddr_in addr;
            addr.sin_family = AF_INET;
            addr.sin_addr.s_addr = htonl(INADDR_ANY);
            addr.sin_port = 0;  // Set to zero, system will asign a port after bind().
            len = sizeof(addr);
            bind(zh->fd, (struct sockaddr *)&addr, sizeof(addr));
            getsockname(zh->fd, (struct sockaddr *) &addr, (socklen_t*)&len);
            zh->port = ntohs(addr.sin_port);

            setsockopt(zh->fd, IPPROTO_TCP, TCP_NODELAY, &on, sizeof(int));
            fcntl(zh->fd, F_SETFL, O_NONBLOCK|fcntl(zh->fd, F_GETFL, 0));
            // sleep for 10 microseconds to avoid connect too fast
            usleep(10000);
            rc = connect(zh->fd, (struct sockaddr*) &zh->addrs[zh->connect_index], sizeof(struct sockaddr_in));
            LOG_DEBUG(("(%#llx) Connect to server [%s], client id [%s:%d], timeout set to: %d",
                        zh->client_id.client_id,
                        format_endpoint_info(&zh->addrs[zh->connect_index]),
                        get_local_ip(), zh->port,
                        zh->recv_timeout));
        ...
        if (recv_to <= 0) {
            ...
            *tv = get_timeval(0);
            return api_epilog(zh,handle_socket_error_msg(zh,
                    ...
                    "connection timed out (exceeded timeout by %dms)", idle_recv));
        }
        // We only allow 1/3 of our timeout time to expire before sending a PING
        if (zh->state==ZOO_CONNECTED_STATE) {
            send_to = zh->recv_timeout/3 - idle_send;
            if (send_to <= 0 && zh->sent_requests.head==0) {
                LOG_DEBUG(("(%#llx) Sending PING to %s (exceeded idle by %dms)",
                            zh->client_id.client_id, format_current_endpoint_info(zh), idle_send));
                int rc=send_ping(zh);
                if (rc < 0){
                    LOG_DEBUG(("(%#llx) failed to send PING request (zk retcode=%d)",
                                zh->client_id.client_id, rc));
                    return api_epilog(zh,rc);
                }
                send_to = zh->recv_timeout/3;
            } else if (send_to <= 0) {
                LOG_DEBUG(("(%#llx) exceeded idle by %dms, not send ping because data already in buffer.",
                            zh->client_id.client_id, idle_send));
            }
        }
    ...
    if (events&ZOOKEEPER_READ) {
        ...
        if (rc > 0) {
            ...
            if (zh->input_buffer != &zh->primer_buffer) {
                ...
                int64_t oldid,newid;
                //deserialize
                deserialize_prime_response(&zh->primer_storage, &zh->primer_buffer);
                ...
                if (zh->primer_storage.error_code != ZOK) {
                    if (zh->primer_storage.error_code == ZAUTHFAILED) {
                        return handle_socket_error_msg(zh, __LINE__, ZAUTHFAILED,
                                    "Auth failed when connect server.");

                    } else if (zh->primer_storage.error_code == ZNOQUOTA) {
                        // no connection quota, try another server.
                        return handle_socket_error_msg(zh, __LINE__ ,ZNOQUOTA,
                                    "No quota left for current role.");
                    } else if (zh->primer_storage.error_code == ZSERVEROVERLOAD) {
                        return handle_socket_error_msg(zh, __LINE__, ZAUTHFAILED,
                                    "Server overloads.");
                    } else {
                        return handle_socket_error_msg(zh, __LINE__, zh->primer_storage.error_code,
                                    "failed to create session, error: %d.",
                                    zh->primer_storage.error_code);
                    }
                }
                if (oldid != 0 && oldid != newid) {
                    ...
                    return handle_socket_error_msg(zh,__LINE__,ZSESSIONEXPIRED,
                            "sessionId=%#llx has expired, new sessionId=%#llx",oldid, newid);
                } else {
                    ...
                    LOG_DEBUG(("(%#llx) session established to [%s], negotiated timeout=%d", newid,
                              format_endpoint_info(&zh->addrs[zh->connect_index]), zh->recv_timeout));
...
static watcher_registration_t* create_watcher_registration(const char* path,
    ...
    if(watcher==0)
        return 0;
    wo = static_cast<watcher_registration_t*>(calloc(1,sizeof(watcher_registration_t)));
    wo->path=strdup(path);
    wo->watcher=watcher;
    wo->context=ctx;
    wo->checker=checker;
...
static completion_list_t* create_completion_entry(int xid, int completion_type,
        const void *dc, const void *data,watcher_registration_t* wo)
{
    completion_list_t *c = static_cast<completion_list_t*>(calloc(1,sizeof(completion_list_t)));
    if (!c) {
        LOG_ERROR(("out of memory"));
        return 0;
    }
...
{
    return add_completion(zh, xid, COMPLETION_DATA, reinterpret_cast<void*>(dc), data, 0,wo);
}

static int add_stat_completion(zhandle_t *zh, int xid, stat_completion_t dc,
        const void *data,watcher_registration_t* wo)
{
    return add_completion(zh, xid, COMPLETION_STAT, reinterpret_cast<void*>(dc), data, 0,wo);
}

static int add_strings_completion(zhandle_t *zh, int xid,
        strings_completion_t dc, const void *data,watcher_registration_t* wo)
{
    return add_completion(zh, xid, COMPLETION_STRINGLIST, reinterpret_cast<void*>(dc), data, 0,wo);
}
...
{
    return add_completion(zh, xid, COMPLETION_STRINGLIST_STAT, reinterpret_cast<void*>(dc), data, 0,wo);
}

static int add_acl_completion(zhandle_t *zh, int xid, acl_completion_t dc,
        const void *data)
{
    return add_completion(zh, xid, COMPLETION_ACLLIST, reinterpret_cast<void*>(dc), data, 0,0);
}

static int add_void_completion(zhandle_t *zh, int xid, void_completion_t dc,
        const void *data)
{
    return add_completion(zh, xid, COMPLETION_VOID, reinterpret_cast<void*>(dc), data, 0,0);
}

static int add_string_completion(zhandle_t *zh, int xid,
        string_completion_t dc, const void *data)
{
    return add_completion(zh, xid, COMPLETION_STRING, reinterpret_cast<void*>(dc), data, 0,0);
}
...
int zookeeper_close(zhandle_t *zh)
{
    ...
    if(zh->state==ZOO_CONNECTED_STATE){
        struct oarchive *oa;
        struct RequestHeader h = { get_xid(), CLOSE_OP };
        LOG_DEBUG(("(%#llx) Closing current zookeeper session to [%s]\n",
        ...
        LOG_DEBUG(("(%#llx) Freeing current zookeeper session resources.\n",
                zh->client_id.client_id));
        rc = ZOK;
    }
...
int isValidPath(const char* path, const int flags, bool create) {
    int len = 0;
    char lastc = '/';
    char c;
    int i = 0;
    ...
    for (; i < len; lastc = path[i], i++) {
        ...
        if (c == 0) {
            ...
            if (create) {
                // only allow '-', '_', '/', '.', 'a'-'z', 'A-Z', '0-9' in zk create path.
        ...
        data_completion_t dc, const void *data)
{
    ...
    struct RequestHeader h = { get_xid(), GETDATA_OP };
    ...
    LOG_DEBUG(("[Get data] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), SETDATA_OP };
    ...
    LOG_DEBUG(("[Set data] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), CREATE_OP };
    struct CreateRequest req;
    int rc;
    char *server_path;

    server_path = prepend_string(zh, path);
    if (zh==0 || !isValidPath(server_path, flags, true)) {
        free_duplicate_path(server_path, path);
        return ZBADARGUMENTS;
    }
    if (is_unrecoverable(zh)) {
    ...
    LOG_DEBUG(("[Create node] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), DELETE_OP };
    ...
    LOG_DEBUG(("[Delete node] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), EXISTS_OP };
    char *server_path = prepend_string(zh, path);
    ...
    LOG_DEBUG(("[Exist node] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), GETCHILDREN_OP };
    char * server_path = prepend_string(zh, path);
    ...
    LOG_DEBUG(("[Get children] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
         ...
         const void *data)
{
    ...
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), GETCHILDREN2_OP };
    char * server_path = prepend_string(zh, path);
    ...
    LOG_DEBUG(("[Get children2] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), SYNC_OP };
    ...
    LOG_DEBUG(("[Sync data] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), GETACL_OP };
    ...
    LOG_DEBUG(("[Get Acl] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
{
    struct oarchive *oa;
    struct RequestHeader h = { get_xid(), SETACL_OP };
    ...
    LOG_DEBUG(("[Set Acl] Sending request xid=%#x for path [%s] to %s",h.xid,path,
            format_current_endpoint_info(zh)));
    /* make a best (non-blocking) effort to send the requests asap */
    adaptor_send_queue(zh, 0);
    return (rc < 0)?ZMARSHALLINGERROR:ZOK;
...
int flush_send_queue(zhandle_t*zh, int timeout)
{
    ...
    while (zh->to_send.head != 0&& zh->state == ZOO_CONNECTED_STATE) {
        ...
        if(rc==0 && timeout==0){
            ...
            LOG_DEBUG(("(%#llx) Get EWOULDBLOCK when sending data.", zh->client_id.client_id));
            rc = ZOK;
            break;
        }
        if (rc < 0) {
    ...
    if (rc != ZOK) {
        LOG_DEBUG(("(%#llx) flush sending queue error:%d", zh->client_id.client_id, rc));
    }
...
const char* zerror(int c)
{
    ...
    case ZNOQUOTA:
      return "connection limit exceeded, try another server in addr list.";
    }
    if (c > 0) {
      return strerror(c);
    }
        ...
        int certLen,void_completion_t completion, const void *data)
{
    ...
    if(cert!=NULL && certLen!=0){
        auth.buff = static_cast<char*>(calloc(1,certLen));
        if(auth.buff==0) {
            return ZSYSTEMERROR;
        }
    ...
    authinfo->data = static_cast<const char*>(data);
    set_auth(&zh->auth_h, authinfo);
    zoo_unlock_auth(zh);
        ...
        char *path_buffer, int path_buffer_len)
{
    ...
    rc=zoo_acreate(zh, path, value, valuelen, acl, flags,
            (string_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
...
int zoo_delete(zhandle_t *zh, const char *path, int version)
{
    ...
    rc=zoo_adelete(zh, path, version, (void_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
    }
        ...
        watcher_fn watcher, void* watcherCtx, struct Stat *stat)
{
    ...
    rc=zoo_awexists(zh,path,watcher,watcherCtx,(stat_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
        if (rc == 0&& stat) {
        ...
        char *buffer, int* buffer_len, struct Stat *stat)
{
    ...
    rc=zoo_awget(zh, path, watcher, watcherCtx, (data_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
        if (rc == 0) {
        ...
        int version, struct Stat *stat)
{
    ...
    rc=zoo_aset(zh, path, buffer, buflen, version, (stat_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
        if (rc == 0 && stat) {
        ...
        struct String_vector *strings)
{
    ...
    rc= zoo_awget_children (zh, path, watcher, watcherCtx,
            (strings_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
        if (rc == 0) {
        ...
        struct String_vector *strings, struct Stat *stat)
{
    ...
    rc= zoo_awget_children2(zh, path, watcher, watcherCtx,
            (strings_stat_completion_t)SYNCHRONOUS_MARKER, sc);
        ...
        struct Stat *stat)
{
    ...
    rc=zoo_aget_acl(zh, path, (acl_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
        if (rc == 0&& stat) {
        ...
        const struct ACL_vector *acl)
{
    ...
    rc=zoo_aset_acl(zh, path, version, (struct ACL_vector*)acl,
            (void_completion_t)SYNCHRONOUS_MARKER, sc);
    if(rc==ZOK){
        wait_sync_completion(sc);
        rc = sc->rc;
    }
