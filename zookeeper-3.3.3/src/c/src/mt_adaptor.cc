...
{
    struct adaptor_threads* adaptor = static_cast<adaptor_threads*>(zh->adaptor_priv);
...
int adaptor_init(zhandle_t *zh)
{
    ...
    struct adaptor_threads *adaptor =
        static_cast<adaptor_threads*>(calloc(1, sizeof(*adaptor)));
    if (!adaptor) {
    ...
    if(pipe(adaptor->self_pipe)==-1) {
        LOG_ERROR(("Can't make a pipe %d",errno));
        free(adaptor);
        return -1;
    }
    set_nonblock(adaptor->self_pipe[1]);
    set_nonblock(adaptor->self_pipe[0]);

    pthread_mutex_init(&zh->auth_h.lock,0);

    zh->adaptor_priv = adaptor;
    pthread_mutex_init(&zh->to_process.lock,0);
    pthread_mutex_init(&adaptor->zh_lock,0);
...
void adaptor_finish(zhandle_t *zh)
{
    ...
    api_prolog(zh); 
    adaptor = static_cast<adaptor_threads*>(zh->adaptor_priv);
    if(adaptor==0) {
        api_epilog(zh,0);
        return;
    }

    if(!pthread_equal(adaptor->io,pthread_self())){
        wakeup_io_thread(zh);
        pthread_join(adaptor->io, 0);
    }else
        pthread_detach(adaptor->io);
    
    if(!pthread_equal(adaptor->completion,pthread_self())){
        pthread_mutex_lock(&zh->completions_to_process.lock);
        pthread_cond_broadcast(&zh->completions_to_process.cond);
        pthread_mutex_unlock(&zh->completions_to_process.lock);
        pthread_join(adaptor->completion, 0);
    }else
        pthread_detach(adaptor->completion);
...
int wakeup_io_thread(zhandle_t *zh)
{
    struct adaptor_threads *adaptor = static_cast<adaptor_threads*>(zh->adaptor_priv);
    char c=0;
    return write(adaptor->self_pipe[1],&c,1)==1? ZOK: ZSYSTEMERROR;    
}
...
void *do_io(void *v)
{
    zhandle_t *zh = static_cast<zhandle_t*>(v);
    struct pollfd fds[2];
    struct adaptor_threads *adaptor = static_cast<adaptor_threads*>(zh->adaptor_priv);
    ...
    while(!zh->close_requested) {
        ...
        if(fds[0].revents&POLLIN){
            // flush the pipe
            char b[128];
            while(read(adaptor->self_pipe[0],b,sizeof(b))==sizeof(b)){}
        }
