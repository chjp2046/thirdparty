...
watcher_object_t* getFirstWatcher(zk_hashtable* ht,const char* path)
{
    watcher_object_list_t* wl =
        static_cast<watcher_object_list_t*>(hashtable_search(ht->ht,(void*)path));
    if(wl!=0)
        return wl->head;
    return 0;
}
...
{
    watcher_object_t* res = static_cast<watcher_object_t*>(calloc(1,sizeof(watcher_object_t)));
    assert(res);
...
{
    watcher_object_t* wo = static_cast<watcher_object_t*>(calloc(1,sizeof(watcher_object_t)));
    assert(wo);
    wo->watcher=watcher;
    wo->context=ctx;
    return wo;
...
{
    watcher_object_list_t* wl =
        static_cast<watcher_object_list_t*>(calloc(1,sizeof(watcher_object_list_t)));
    assert(wl);
    wl->head=head;
    return wl;
}
...
{
    struct _zk_hashtable *ht = static_cast<_zk_hashtable*>(calloc(1,sizeof(struct _zk_hashtable)));
    assert(ht);
...
static void do_clean_hashtable(zk_hashtable* ht)
{
    ...
    do {
        watcher_object_list_t* w =
            static_cast<watcher_object_list_t*>(hashtable_iterator_value(it));
...
static int do_insert_watcher_object(zk_hashtable *ht, const char *path, watcher_object_t* wo)
{
    ...
    wl = static_cast<watcher_object_list_t*>(hashtable_search(ht->ht,(void*)path));
    if(wl==0){
        int res;
...
char **collect_keys(zk_hashtable *ht, int *count)
{
    ...
    *count = hashtable_count(ht->ht);
    list = static_cast<char**>(calloc(*count, sizeof(char*)));
    it=hashtable_iterator(ht->ht);
    for(i = 0; i < *count; i++) {
        list[i] = strdup(static_cast<const char*>(hashtable_iterator_key(it)));
