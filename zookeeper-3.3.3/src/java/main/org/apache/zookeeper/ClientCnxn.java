...
public class ClientCnxn {
    ...
    public ClientCnxn(String hosts, int sessionTimeout, ZooKeeper zooKeeper,
            ClientWatchManager watcher, CredentialGenerator generator)
        throws IOException
    {
        this(hosts, sessionTimeout, zooKeeper, watcher, 0, new byte[16], generator);
    }
    ...
    class SendThread extends Thread {
        ...
        private void primeConnection(SelectionKey k) throws IOException {
            ...
            byte[] credential_value = null;
            if (generator != null) {
                string_p credential = new string_p();
                if (!generator.Generate(credential.cast())) {
                    LOG.error("generator credential error, use guest login.");
                    credential_value = new byte[0];
                } else {
                    credential_value = credential.value();
                }
            } else {
                // no generator, set credential to empty
                credential_value = new byte[0];
            }
            ConnectRequest conReq = new ConnectRequest(0, lastZxid,
                    sessionTimeout, sessionId, sessionPasswd, credential_value);
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
