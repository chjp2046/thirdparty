...
public class DataTree {
    ...
    /**
     * the zookeeper role based quota node that acts as the quota management node for
     * zookeeper
     */
    private static final String roleQuotaZookeeper = RoleRelatedQuotas.quotaZookeeper;

    /** this will be the string thats stored as a child of /zookeeper */
    private static final String roleQuotaChildZookeeper = roleQuotaZookeeper
            .substring(procZookeeper.length() + 1);
    ...
    /**
     * create a /zookeeper/role node for maintaining role based quota properties
     * for zookeeper.
     */
    // (TODO) set ACL on this node to protected quota node.
    private DataNode quotaRoleNode = new DataNode(procDataNode, new byte[0],
            -1L, new StatPersisted());
    ...
    public DataTree() {
        ...
        // add /zookeeper/role for quota
        procDataNode.addChild(roleQuotaChildZookeeper);
        nodes.put(roleQuotaZookeeper, quotaRoleNode);
    }
    ...
    // updated quota Stat.
    public void updateSessionCountForRole(String role, int diff) {
        // zk_admin do NOT updated session count.
        if (role.equals("zk_admin")) {
            return;
        }
        String statNode = RoleRelatedQuotas.statPath(role);
        DataNode node = nodes.get(statNode);
        if (node == null) {
            // should not happen
            LOG.error("Missing quota count node for stat " + statNode);
            return;
        }
        synchronized (node) {
            String sessionCountStr = new String(node.data);
            int sessionCount = 0;
            try {
                sessionCount = Integer.parseInt(sessionCountStr);
            } catch (Exception e) {
                LOG.error("Quota count node do NOT have a integer data.");
                return;
            }
            LOG.debug("oldSessionCount: " + sessionCount);
            sessionCount += diff;
            if (sessionCount < 0) {
                LOG.error("Quota stat of role: " + role + " below zero.");
                sessionCount = 0;
            }
            node.data = (sessionCount + "").getBytes();
            LOG.debug("newSessionCount: " + new String(node.data));
        }
    }
    ...
    public String createNode(String path, byte data[], List<ACL> acl,
        ...
        LOG.info("[OPERATION:CREATE]: " + path);
        return path;
    }
    ...
    public void deleteNode(String path, long zxid)
        ...
        LOG.info("[OPERATION:DELETE]: " + path);
    }
    ...
    public Stat setData(String path, byte data[], int version, long zxid,
        ...
        LOG.info("[OPERATION:SETDATA]: " + path);
        return s;
    }
    ...
    public Stat setACL(String path, List<ACL> acl, int version)
        ...
        synchronized (n) {
            ...
            n.copyStat(stat);
            LOG.info("[OPERATION:SETACL]: " + path);
            return stat;
        }
    }
        ...
        try {
            ...
            switch (header.getType()) {
                ...
                case OpCode.createSession:
                    // if deserialize createSessionTxn from the old version of snapshot and
                    // transaction log, role will be guest.
                    CreateSessionTxn createSessionTxn = (CreateSessionTxn) txn;
                    String createSessionRole = createSessionTxn.getRole();
                    LOG.debug("createSession use role: " + createSessionRole);
                    updateSessionCountForRole(createSessionRole, 1);
                    break;
                case OpCode.closeSession:
                    String deleteSessionRole = "guest";
                    // old version snapshot do not store closeSession txn, so txn may be null.
                    if (txn != null) {
                        DeleteSessionTxn deleteSessionTxn = (DeleteSessionTxn) txn;
                        deleteSessionRole = deleteSessionTxn.getRole();
                    }
                    LOG.debug("deleteSession use role: " + deleteSessionRole);
                    updateSessionCountForRole(deleteSessionRole, -1);
