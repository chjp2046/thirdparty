    ...
    protected void pRequest(Request request) {
        ...
        try {
            ...
            case OpCode.createSession:
                ...
                // (TODO) use ZooKeeperServer.byteBuffer2Record instead.
                request.request.rewind();
                int to = request.request.getInt();
                int createRoleLen = request.request.remaining();
                byte[] b = new byte[createRoleLen];
                request.request.get(b, 0, b.length);
                String createSessionRole = new String(b, "UTF8");
                txn = new CreateSessionTxn(createSessionRole, to);
                request.request.rewind();
                zks.sessionTracker.addSession(request.sessionId, to, createSessionRole);
                zks.setOwner(request.sessionId, request.getOwner());
                break;
            case OpCode.closeSession:
                txnHeader = new TxnHeader(request.sessionId, request.cxid, zks
                        .getNextZxid(), zks.getTime(), OpCode.closeSession);
                // closeSession request comes from client, and direct send to leader.
                // we can get role from cnxn.
                if (request.cnxn != null) {
                    txn = new DeleteSessionTxn(request.cnxn.getRole());
                // closeSession request comes from client, but is forwarded by follow.
                // we can get role from the authInfo
                } else if (request.authInfo != null && request.authInfo.size() > 0) {
                    String closeRole = request.authInfo.get(0).getId().split(":")[0];
                    txn = new DeleteSessionTxn(closeRole);
                // closeSession request generate by server because of session expired.
                // we can get role from request body.
                } else if (request.request != null) {
                    // (TODO) use ZooKeeperServer.byteBuffer2Record instead.
                    request.request.rewind();
                    int closeRoleLen = request.request.remaining();
                    byte[] closeRoleBytes = new byte[closeRoleLen];
                    request.request.get(closeRoleBytes, 0, closeRoleBytes.length);
                    String closeRole = new String(closeRoleBytes, "UTF8");
                    request.request.rewind();
                    txn = new DeleteSessionTxn(closeRole);
                }
