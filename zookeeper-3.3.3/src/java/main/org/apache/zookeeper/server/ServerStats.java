...
public class ServerStats {
    ...
    public class StatsDumper extends Thread {
        ...
        private final int INTERVAL = 1000; // Record each second.
        volatile boolean running = true;

        public StatsDumper(Provider provider) {
            this.provider = provider;
        }
        @Override
        synchronized public void run() {
            long currentTime = System.currentTimeMillis();
            long nextRecordTime = currentTime;
            try {
                while (running) {
                    currentTime = System.currentTimeMillis();
                    if (nextRecordTime > currentTime) {
                        this.wait(nextRecordTime - currentTime);
                        continue;
                    }
                    String stat = null;
                    if (provider != null) {
                    ...
                    nextRecordTime += INTERVAL;
                }
            } catch (InterruptedException e) {
                LOG.error("Unexpected interruption", e);
            }
            LOG.warn("StatsDumper exited loop!");
        }
        public void shutdown() {
            running = false;
        }
    }
    private StatsDumper dumper = null;
    ...
    public ServerStats(Provider provider) {
        this.provider = provider;
        dumper = new StatsDumper(provider);
    }
    ...
    // mutators
    synchronized void updateLatency(long latency) {
