...
import org.apache.zookeeper.server.counter.RoleOpCounter;
import org.apache.zookeeper.server.counter.LatencyStats;
...
public class ZooKeeperServer implements SessionExpirer, ServerStats.Provider {
    ...
    public final TimedStats connectStats = new TimedStats("Connect\t");
    public final TimedStats closeStats = new TimedStats("Close\t");
    public final TimedStats getStats = new TimedStats("GetData\t");
    public final TimedStats setStats = new TimedStats("SetData\t");
    public final TimedStats createStats = new TimedStats("Create\t");
    public final TimedStats deleteStats = new TimedStats("Delete\t");
    public final TimedStats createSessionStats = new TimedStats("CreateSession");
    public final TimedStats deleteSessionStats = new TimedStats("DeleteSession");
    public final LatencyStats latencyStats = new LatencyStats("Latency");
    public final RoleOpCounter roleOpCounter = new RoleOpCounter();
    public long totalAuthorFailed = 0;
    public long totalNoQuota = 0;
    public long totalOverload = 0;

    public String dump() {
        StringBuilder sb = new StringBuilder();
        if (serverCnxnFactory != null) {
            sb.append("Client number: " + serverCnxnFactory.getCnxnCount() + "\n");
        }
        sb.append(serverStats().toString());
        if (zkDb != null) {
            sb.append("Node count: " + zkDb.getNodeCount() + "\n");
        }

        sb.append("=============================================\n");
        sb.append("StatsName\tSecond\tMinute\tHour\tTotal\n");
        sb.append("=============================================\n");
        sb.append(connectStats.toString());
        sb.append(closeStats.toString());
        sb.append(getStats.toString());
        sb.append(setStats.toString());
        sb.append(createStats.toString());
        sb.append(deleteStats.toString());
        sb.append(createSessionStats.toString());
        sb.append(deleteSessionStats.toString());
        //sb.append(serverCnxnFactory.statsToString());

        sb.append("=============================================\n");
        sb.append("Latency\t\tMin\tMax\tAverage\n");
        sb.append("=============================================\n");
        sb.append(latencyStats.toString());
        return sb.toString();
    }
    ...
    public void dumpConf(PrintWriter pwriter) {
        ...
        pwriter.print("maxSessionCreatedPerSecond=");
        pwriter.println(serverCnxnFactory.getMaxSessionCreatedPerSecond());
    ...
    private void close(long sessionId, String role) {
        // session close operation occur in two ways. one is session closed by client,
        // others is session expired by server. this method called when session expired,
        // the param role get from the expired SessionInfo, create a request and the
        // request body contains the role of expired session.
        ByteBuffer roleBb = ByteBuffer.allocate(role.length());
        roleBb.put(role.getBytes());
        submitRequest(null, sessionId, OpCode.closeSession, 0, roleBb, null);
    }

    public void closeSession(long sessionId, String role) {
        LOG.info("Closing session 0x" + Long.toHexString(sessionId));

        // we do not want to wait for a session close. send it as soon as we
        // detect it!
        close(sessionId, role);
    }
    ...
    // sessionTracker call this method to deal with expired session.
    public void expire(Session session) {
        long sessionId = session.getSessionId();
        LOG.info("Expiring session 0x" + Long.toHexString(sessionId)
                + ", timeout of " + session.getTimeout() + "ms exceeded");
        close(sessionId, session.getRole());
    }
    ...
    long createSession(ServerCnxn cnxn, byte passwd[], int timeout)
        ...
        long sessionId = sessionTracker.createSession(timeout, cnxn.getRole());
        Random r = new Random(sessionId ^ superSecret);
        r.nextBytes(passwd);
        ByteBuffer createSessionBb = ByteBuffer.allocate(4 + cnxn.getRole().length());
        createSessionBb.putInt(timeout);
        createSessionBb.put(cnxn.getRole().getBytes());
        cnxn.setSessionId(sessionId);
        submitRequest(cnxn, sessionId, OpCode.createSession, 0, createSessionBb, null);
        return sessionId;
    }
    ...
    public void closeSession(ServerCnxn cnxn, RequestHeader requestHeader) {
        closeSession(cnxn.getSessionId(), cnxn.getRole());
    }
    ...
    public void killSessionByRole(String role) {
        List<Long> roleSessions = ((SessionTrackerImpl)sessionTracker).getSessionByRole(role);
        for (Long sessionId : roleSessions) {
            this.closeSession(sessionId, role);
        }
    }
}
