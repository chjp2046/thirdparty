...
import org.apache.zookeeper.server.Bool;
import org.apache.zookeeper.server.auth.AuthenticationProvider;
import org.apache.zookeeper.server.auth.ProviderRegistry;
import org.apache.zookeeper.server.counter.TimedStats;
...
public class NIOServerCnxn implements Watcher, ServerCnxn {
    ...
    static public class Factory extends Thread {
        ...
        */
        ...
        int maxSessionCreatedPerSecond = 100;

        final HashMap<String, TimedStats> roleStats = new HashMap<String, TimedStats>();
        ...
        public boolean checkSessions(String role, long timestamp) {
            synchronized (this.roleStats) {
                TimedStats stats = roleStats.get(role);
                if (stats == null) {
                    stats = new TimedStats(role);
                    stats.addCount(timestamp);
                    roleStats.put(role, stats);
                    return true;
                } else {
                    return stats.addCountIfSecondCountLT(maxSessionCreatedPerSecond, timestamp);
                }
            }
        }

        public long getSecondCreateSessionCount(String role, long timestamp) {
            synchronized (this.roleStats) {
                TimedStats stats = roleStats.get(role);
                if (stats == null) {
                    return 0;
                } else {
                    return stats.getSecondCount(timestamp);
                }
            }
        }
        ...
        public String statsToString() {
            ...
            synchronized (this.roleStats) {
                ArrayList<String> keys = new ArrayList<String>(roleStats.keySet());
                for (String role : keys) {
                    sb.append(roleStats.get(role).toString());
                }
            }
            return sb.toString();
        }
         ...
         * @throws IOException
         */
        public Factory(InetSocketAddress addr, int maxcc, int maxsc) throws IOException {
            super("NIOServerCxn.Factory:" + addr);
            setDaemon(true);
            maxClientCnxns = maxcc;
            maxSessionCreatedPerSecond = maxsc;
        ...
        public int getMaxSessionCreatedPerSecond() {
            return maxSessionCreatedPerSecond;
        }
        ...
        public void run() {
                    ...
                    for (SelectionKey k : selectedList) {
                        if ((k.readyOps() & SelectionKey.OP_ACCEPT) != 0) {
                            ...
                            if (maxClientCnxns > 0 && cnxncount >= maxClientCnxns){
                                ...
                                LOG.warn("Accepted socket connection from "
                                ...
                                try {
                                    if (zks != null) {
                                        zks.connectStats.addCount(timestamp);
                                    }
                                }
                                catch (Exception e) {}
                            }
            ...
            clear();
            LOG.warn("NIOServerCnxn factory exited run method");
        }
    ...
    // indicate if this cnxn is reopen a session.
    boolean reopenOldSession = false;

    Bool oldClient = new Bool();
    ...
    void sendBuffer(ByteBuffer bb) {
        try {
            ...
            LOG.error("Session 0x" + Long.toHexString(sessionId) + " is invalid now.");
        } catch(Exception e) {
    ...
    private void readRequest() throws IOException {
        ...
        if (h.getType() == OpCode.auth) {
            ...
            if (ap == null
                ...
                // first login and failure, close connection
                // sucessful login before, keep it's role and the connection.
                if (this.role.equals("")) {
                    // send a response...
                ...
                // (TODO) check the quota of new role when change role sucessfully.
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Authentication succeeded for scheme: "
                              + scheme + " role:" + this.role);
                }
    ...
    private void readConnectRequest() throws IOException, InterruptedException {
        ...
        connReq.deserialize(bia, "connect", oldClient);
        ...
        byte credential[] = connReq.getCredential();
        ...
        setReopenOldSession(connReq.getSessionId() != 0);

        // authentication.
        AuthenticationProvider ap = null;
        if (System.getProperty("zookeeper.disableTaasAuthentication", "no").equals("yes")) {
            ap = ProviderRegistry.getProvider("digest");
        } else {
            ap = ProviderRegistry.getProvider("taas");
        }
        if (ap == null) {
            LOG.warn("Auth provider is null.");
        }
        KeeperException.Code authenRet = KeeperException.Code.AUTHFAILED;
        if (ap == null || (authenRet = ap.handleAuthentication(this, credential))
                == KeeperException.Code.AUTHFAILED) {
            ++zk.totalAuthorFailed;
            String msg = "Refusing create session request from client "
                + sock.socket().getRemoteSocketAddress()
                + " for the reason of AUTHFAILED";

            LOG.warn(msg);
            sendErrorResponse(KeeperException.Code.AUTHFAILED.intValue());
            throw new CloseRequestException(msg);
        } else if (KeeperException.Code.NOQUOTA == authenRet) {
            ++zk.totalNoQuota;
            String msg = "Refusing create session request from client "
                + sock.socket().getRemoteSocketAddress()
                + " because role [" + this.role + "] has NOQUOTA";
            LOG.warn(msg);
            sendErrorResponse(KeeperException.Code.NOQUOTA.intValue());
            throw new CloseRequestException(msg);
        }

        // throttling control
        // for new established connection, if create session requests for a role
        // exceed the max threshold, we reject current request directly.
        if (connReq.getSessionId() == 0) {
            long timestamp = System.currentTimeMillis() / 1000;
            if (!factory.checkSessions(this.role, timestamp)) {
                ++zk.totalOverload;
                String msg = "Refusing create session request from client "
                    + sock.socket().getRemoteSocketAddress()
                    + ", role: " + this.role
                    + " for throttling control, now createSesssion queue size: "
                    + factory.getSecondCreateSessionCount(this.role, timestamp);
                LOG.warn(msg);
                sendErrorResponse(KeeperException.Code.SERVEROVERLOAD.intValue());
                throw new CloseRequestException(msg);
            }
        }
        ...
        if (connReq.getSessionId() != 0) {
            ...
            LOG.warn("Client attempting to reopen old session 0x"
            ...
            LOG.warn("Client attempting to establish new session: "
                    + sock.socket().getRemoteSocketAddress() + ", role: " + this.role);
     ...
     * Zk Admin</a>. this link is for all the commands.
     */
    private final static int quotCmd =
        ByteBuffer.wrap("quot".getBytes()).getInt();

    /*
     * See <a href="{@docRoot}/../../../docs/zookeeperAdmin.html#sc_zkCommands">
     * Zk Admin</a>. this link is for all the commands.
     */
    private final static int reptCmd =
        ByteBuffer.wrap("rept".getBytes()).getInt();
    ...
    static {
        ...
        cmd2String.put(quotCmd, "quot");
        cmd2String.put(reptCmd, "rept");
    }
    ...
    private class StatCommand extends CommandThread {
        ...
        public void commandRun() {
            if (zk == null) {
                ...
                pw.println("=============================================");
                pw.print("StatsName\tSecond\tMinute\tHour\tTotal\n");
                pw.println("=============================================");
                pw.print(zk.connectStats.toString());
                pw.print(zk.closeStats.toString());
                pw.print(zk.getStats.toString());
                pw.print(zk.setStats.toString());
                pw.print(zk.createStats.toString());
                pw.print(zk.deleteStats.toString());
                pw.print(zk.createSessionStats.toString());
                pw.print(zk.deleteSessionStats.toString());

                pw.println("=============================================");
                pw.print("Latency\t\tMin\tMax\tAverage\n");
                pw.println("=============================================");
                pw.print(zk.latencyStats.toString());
                pw.println("Total author failed:\t" + zk.totalAuthorFailed);
                pw.println("Total noQuota:\t\t" + zk.totalNoQuota);
                pw.println("Total overload:\t\t" + zk.totalOverload);
            }
        }
    }
    ...
    private class QuotaCommand extends CommandThread {
        int len = 0;
        public QuotaCommand(PrintWriter pw) {
            super(pw);
        }
        ...
        public void commandRun() {
            if (zk == null) {
                ...
                pw.print("StatsName\tSecond\tMinute\tHour\tTotal\n");
                pw.println("=============================================");
                pw.print(factory.statsToString());
            }
        }
    }

    private class ReportCommand extends CommandThread {
        public ReportCommand(PrintWriter pw) {
            super(pw);
        }
        @SuppressWarnings("unchecked")
        @Override
        public void commandRun() {
            if (zk == null) {
                ...
                HashMap<String, Integer> roleActiveCountMap = new HashMap<String, Integer>();
                for (NIOServerCnxn cnxn : cnxnset) {
                    String role = cnxn.getRole();
                    Integer count = roleActiveCountMap.get(role);
                    if (count == null) {
                        count = 1;
                    } else {
                        count += 1;
                    }
                    roleActiveCountMap.put(role, count);
                }
                pw.print(zk.roleOpCounter.dump(roleActiveCountMap));
            }
        }
    }
    ...
    throws IOException
    {
        ...
        if (len == ruokCmd) {
            ...
            QuotaCommand quot = new QuotaCommand(pwriter);
            quot.start();
            return true;
        } else if (len == reptCmd) {
            ReportCommand rept = new ReportCommand(pwriter);
            rept.start();
            return true;
        }
        return false;
    ...
    public void close() {
        synchronized(factory.cnxns){
            ...
            if (zk != null) {
                zk.removeCnxn(this);
                long timestamp =  System.currentTimeMillis() / 1000;
                zk.closeStats.addCount(timestamp);
            }
    ...
    private void closeSock() {
        ...
        LOG.warn("Closed socket connection for client "
                + sock.socket().getRemoteSocketAddress()
                + (sessionId != 0 ?
    ...
    synchronized public void sendResponse(ReplyHeader h, Record r, String tag) {
        try {
            ...
            LOG.warn("Unexpected exception when seding response.");
         }
    }
    ...
    public void sendErrorResponse(int type) {
        try {
            ConnectResponse rsp = new ConnectResponse(0, 0, 0, new byte[16], type);
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            BinaryOutputArchive bos = BinaryOutputArchive.getArchive(baos);
            bos.writeInt(-1, "len");
    ...
    public void finishSessionInit(boolean valid) {
        ...
        try {
            ConnectResponse rsp = new ConnectResponse(0, valid ? sessionTimeout
                    ...
                    valid ? KeeperException.Code.OK.intValue() : 0);
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            BinaryOutputArchive bos = BinaryOutputArchive.getArchive(baos);
            bos.writeInt(-1, "len");
            rsp.serialize(bos, "connect", oldClient);
            baos.close();
            ByteBuffer bb = ByteBuffer.wrap(baos.toByteArray());
            bb.putInt(bb.remaining() - 4).rewind();
            sendBuffer(bb);
    ...
    public void setReopenOldSession(boolean reopenOldSession) {
        this.reopenOldSession = reopenOldSession;
    }

    public boolean getReopenOldSession() {
        return this.reopenOldSession;
    }
